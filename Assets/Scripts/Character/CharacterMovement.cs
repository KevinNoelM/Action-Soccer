...
public class CharacterMovement : MonoBehaviour
{
    ...
    {
        //
        if (IsPlayerControlled() && input.PressFire())
        {
        ...
        else
        {
            ...
            {
                inputDirection = OutOfPossessionDirection();
            }
            if (inputDirection != Vector3.zero)
            {
                ...
                if (input.HoldDash())
                {
                    chestBody.AddForce(chestBody.transform.forward * sprintForce);
                }
            }
            else
            {
    ...
    protected virtual Vector3 OutOfPossessionDirection()
    {
        return Vector3.zero;
    }
    ...
    private void GetUpFromJump()
    {
        ...
        //
        getUpCounter = getUpTime; // ** JUST USED TO SETTLE THE ARMS ***
        //
        //
